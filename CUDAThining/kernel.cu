...
__device__ double B(double *picture, int x, int y, size_t pitch)        //Метод В(Р) возвращает количество черных пикселей в окрестности точки Р
{
...
__device__ double A(double *picture, int x, int y, size_t pitch)        //Метод А(Р) возвращает количество подряд идущих белых и черных пикселей вокруг точки Р (..0->1..)
{
	int counter = 0;
